...
export async function postCallSummaryToSlack(opts: SlackPostOpts): Promise<void> {
  ...
  const sentimentText = sentimentAvg === null
    ? 'Sentiment: _n/a_'
  ...
  try {
    const res = await fetchWithTimeout(opts.webhookUrl, {
      method: 'POST',
